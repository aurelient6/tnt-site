...
import { getAllServicesBasicInfo } from '../../app/data/servicesData.js';
...
const services = getAllServicesBasicInfo();
...
async function seed() {
  try {
    ...
    for (const service of services) {
      await sql`
        INSERT INTO services (slug, name, duration, capacity)
        VALUES (${service.slug}, ${service.name}, ${service.duration}, ${service.capacity})
        ON CONFLICT (slug) DO UPDATE 
        SET name = ${service.name}, duration = ${service.duration}, capacity = ${service.capacity}
      `;
      console.log(`   ✅ Service ajouté: ${service.name} (capacity: ${service.capacity})`);
    }
